...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    {
    #if (GTP_SLIDE_WAKEUP || GTP_GESTURE_WAKEUP)
...
static s32 gtp_init_panel(struct i2c_client *client)
{
...
#endif
    {
        ...
        if (ret == SUCCESS)
        {
            ...
            /* Vanzo:zhangxinyu on Fri 3 Jul 17:30:00 +8000
             *update TP fw do not check TP fw version
             if (opr_buf[0] < 90)
             */
            if (1/*opr_buf[0] < 90*/)
...
/* Vanzo:wangfei on: Fri, 06 Mar 2015 15:19:30 +0800
...
static DEVICE_ATTR(gesture, 0777, gt9xx_gesture_show, gt9xx_gesture_store);
...
static DEVICE_ATTR(coordinate_x, 0777, goodix_coordinate_x_show, NULL);
...
static DEVICE_ATTR(coordinate_y, 0777, goodix_coordinate_y_show, NULL);
#endif
...
static ssize_t gt9xx_fwversion_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    ...
    {  
        ...
        num_read_chars = snprintf(buf, PAGE_SIZE, "IC VERSION: %c%c%c_%d\n",reg[2], reg[3], reg[4], config_reg[2]);
    }
    else
    {
        //num_read_chars = snprintf(buf, PAGE_SIZE, "IC VERSION:%c%c%c%c_%02x%02x\n",reg[2], reg[3], reg[4], reg[5], reg[7], reg[6]);
        num_read_chars = snprintf(buf, PAGE_SIZE, "IC VERSION:%c%c%c%c_%d\n",reg[2], reg[3], reg[4], reg[5], config_reg[2]);
    }
    return num_read_chars;
}
...
#ifdef VANZO_DEVICE_NAME_SUPPORT
{
    ...
    v_set_dev_name(2, "GT9XX_hotknot");
}
#endif
...
/* Vanzo:wangfei on: Fri, 06 Mar 2015 15:19:51 +0800
 * port from zhangqingzhan
 */
#if defined(GPIO_CTP_EN_PIN)
// End of Vanzo:wangfei
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
	...
	GTP_INFO("%s line %d\n",__func__,__LINE__);
        if (DOZE_ENABLED == doze_status && tpd_halt == 1)
        {
            ...
            if (ret > 0)
            {     
                GTP_DEBUG("YangZhihong %s line %d doze_buf[2] %d\n",__func__,__LINE__,doze_buf[2]);    
                isValuedKey = TRUE;
                ...
                if(isValuedKey){
                    GTP_DEBUG("YangZhihong %s line %d\n",__func__,__LINE__);
                    /**/
                    doze_buf[0] = 0x81;
                    doze_buf[1] = 0x4C;
                    ret = gtp_i2c_read(i2c_client_point, doze_buf, 3);
                    ...
                    {							
                        ...
                        GTP_DEBUG("YangZhihong %s line %d gpointnum %d\n",__func__,__LINE__,gpointnum);
                        for(i=0; i< gpointnum; i++)
                        {
                            ...
                            GTP_DEBUG("YangZhihong %s line %d ges_data 0x%x 0x%x, 0x%x 0x%x, 0x%x 0x%x\n",
...
/* Vanzo:wangfei on: Fri, 06 Mar 2015 15:20:14 +0800
 * port from zhangqingzhan
 */
#if defined(GPIO_CTP_EN_PIN) //ifdef MT6573
// End of Vanzo:wangfei
    mt_set_gpio_mode(GPIO_CTP_EN_PIN, GPIO_CTP_EN_PIN_M_GPIO);
    mt_set_gpio_dir(GPIO_CTP_EN_PIN, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_CTP_EN_PIN, GPIO_OUT_ZERO);  
