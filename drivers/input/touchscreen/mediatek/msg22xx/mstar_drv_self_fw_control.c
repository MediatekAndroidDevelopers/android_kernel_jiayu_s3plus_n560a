...
//extern unsigned char msg22xx_sy_update_bin[];
...
//                    input_report_key(g_InputDevice, RESERVER51, 0);
		...
		if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF) && (pPacket[6] == 0xFF))
		{
			if (tpd_proximity_flag == 1)
			{
				...
				DBG("MStar proximity sensor enble _DrvFwCtrlParsePacket pPacket[5]=%d\n", pPacket[5]);
				if(pPacket[5]==0x40||pPacket[5]==0x80)
				{
			
					if (pPacket[5]==0x40)// leave
					{
						tpd_proximity_detect = 1;
					}
					else if (pPacket[5]==0x80)// close to
					{
...
#ifdef TPD_MSTAR_PROXIMITY_SENSOR
int DrvFwCtrl_MStar_Proximity_sensor(int enable)
{
    DBG("*** %s() ***\n", __func__);
	...
	if (enable)
	{
		szDbBusTxData[3] = 0xa0;//0xa2//0xa4
	...
	else
	{
		...
		while (i < 5)
		{
			...
			if (rc > 0)
			{
				if(szDbBusTxData[3]== 0xa0)
					DBG("Enable MStar proximity sensor success\n");
				else if(szDbBusTxData[3]== 0xa1)
					DBG("Disable MStar proximity sensor success\n");
				break;
			}
		...
		if (i == 5)
		{
			DBG("DrvFwCtrl_MStar_Proximity_sensor failed\n");		
			return -1;
		}
		return 0;
}
