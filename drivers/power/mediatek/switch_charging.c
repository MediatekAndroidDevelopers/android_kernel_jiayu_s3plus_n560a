...
#define APPLE_DEBUG_CODE

#define APPLE_DEVICE "APPLE"
#ifdef APPLE_DEBUG_CODE
#undef APPLE_DEBUG
#define APPLE_DEBUG(a,arg...) printk(APPLE_DEVICE ": " a, ##arg)
#define APPLE_FUNC()	printk(APPLE_DEVICE ": %s line=%d\n", __func__, __LINE__)
#else
#define APPLE_DEBUG(arg...)
#define APPLE_FUNC()
#endif
#endif
...
#if defined(VANZO_COMMON_APPLE_CHARGING)
static DEFINE_MUTEX(apple_mutex);

extern kal_bool upmu_is_chr_det(void);
extern void apple_android_only(void);
extern void apple_android_iphone_idle(void);
extern void apple_android_iphone_charging(void);
extern void apple_android_iphone_communication(void);
extern void apple_android_iphone_communication_charging(void);
extern void apple_ac_android_charging(void);
extern void apple_ac_iphone_charging(void);
void apple_pchr_turn_on_charging(BOOL is_enable)
{
	kal_uint32 charging_enable = is_enable;

	/* enable/disable charging */
	battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
}
void apple_android_only_action(void)
{
	APPLE_DEBUG("%s\n ",__func__);
	apple_android_only();
}
void apple_android_iphone_idle_action(void)
{
	APPLE_DEBUG("%s\n ",__func__);
	apple_android_iphone_idle();
}
void apple_android_iphone_charging_action(void)
{
	APPLE_DEBUG("%s\n ",__func__);
	apple_android_iphone_charging();
}
void apple_android_iphone_communication_action(void)
{
	APPLE_DEBUG("%s\n ",__func__);
	apple_android_iphone_communication();
}
void apple_android_iphone_communication_charging_action(void)
{
	APPLE_DEBUG("%s\n ",__func__);
	apple_android_iphone_communication_charging();
}
void apple_ac_android_charging_action(void)
{
	APPLE_DEBUG("%s\n ",__func__);
	apple_ac_android_charging();
}
void apple_ac_iphone_charging_action(void)
{
	APPLE_DEBUG("%s\n ",__func__);
	apple_ac_iphone_charging();
}
void apple_charging_algorithm(void)
{
	mutex_lock(&apple_mutex);

	switch (BMT_status.apple_charging_state) {
	case ANDROID_ONLY:
		apple_android_only_action();
		if(upmu_is_chr_det() == KAL_TRUE)
		{
			BMT_status.apple_charging_state=AC_ANDROID_CHARGING;
		}
		break;

	case ANDROID_IPHONE_IDLE:
		apple_android_iphone_idle_action();
		break;

	case ANDROID_IPHONE_CHARGING:
		apple_android_iphone_charging_action();
		break;

	case ANDROID_IPHONE_COMMUNICATION:
		apple_android_iphone_communication_action();
		break;

	case ANDROID_IPHONE_COMMUNICATION_CHARGING:
		apple_android_iphone_communication_charging_action();
		break;

	case AC_ANDROID_CHARGING:
		apple_ac_android_charging_action();
		if(upmu_is_chr_det() == KAL_FALSE)
		{
			BMT_status.apple_charging_state=ANDROID_ONLY;
		}
		break;

	case AC_IPHONE_CHARGING:
		apple_ac_iphone_charging_action();
		break;
	}
	mutex_unlock(&apple_mutex);
}
void apple_set_charging_state(INT32 event)
{
	APPLE_DEBUG("%s event=%x BMT_status.apple_charging_state=%x\n ",__func__,event,BMT_status.apple_charging_state);
	if(event==USB_PLUG_IN_EVENT)
	{
		BMT_status.charger_exist = KAL_TRUE;
	}
	else if(event==USB_PLUG_OUT_EVENT)
	{
		BMT_status.charger_exist = KAL_FALSE;
	}
	switch (BMT_status.apple_charging_state) {
	case ANDROID_ONLY:
		if(event==HALL_NERA_EVENT)
		{
			BMT_status.apple_charging_state=ANDROID_IPHONE_COMMUNICATION;
		}else if(event==USB_PLUG_IN_EVENT)
		{
			BMT_status.apple_charging_state=AC_ANDROID_CHARGING;
		}
		break;
	...
	case ANDROID_IPHONE_IDLE:
		if(event==HALL_FAR_EVENT)
		{
			BMT_status.apple_charging_state=ANDROID_ONLY;
		}else if(event==USB_PLUG_IN_EVENT)
		{
			if(BMT_status.SOC<10)
			{
				BMT_status.apple_charging_state=AC_ANDROID_CHARGING;
			}
			else
			{
				BMT_status.apple_charging_state=AC_IPHONE_CHARGING;
			}
		}
		break;
	...
	case ANDROID_IPHONE_COMMUNICATION:
		...
		{
			//BMT_status.apple_charging_state=ANDROID_IPHONE_COMMUNICATION_CHARGING;
		}
		break;
	...
	case AC_IPHONE_CHARGING:
		...
		{
			BMT_status.apple_charging_state=ANDROID_IPHONE_IDLE;
		}
		break;
	}
}
