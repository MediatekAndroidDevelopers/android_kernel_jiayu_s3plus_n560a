...
#define PA12_DRIVER_VERSION_C		"1.89.1.02"
/*----------------------------------------------------------------------------*/
	...
	if(enable == 1)
	{
		...
		if(obj->hw->polling_mode_ps == 0)
		{
			...
			{						
				...
				{
            				mt_eint_set_polarity(CUST_EINT_ALS_NUM,CUST_EINTF_TRIGGER_LOW);							
...
/********************************************************************/
int pa122_read_ps(struct i2c_client *client, u16 *data)
{
    struct pa122_priv *obj = i2c_get_clientdata(client);
	int res;
        u16 ps_data = 0;
	...
	res = hwmsen_read_byte(client, REG_PS_DATA, &ps_data);
/*
 * function for value max to 1023
 * Vanzo:xuechuanzheng 20140922
 */
	*data = ps_data << 2;
	if(*data >= 1020)
		*data = 1023;
	...
	switch (PA12_INT_TYPE)
	{
		case 1: /* Hysteresis Type */
			if(psdata > obj->ps_thrd_high){
	 			intr_flag = 0;
				mt_eint_set_polarity(CUST_EINT_ALS_NUM, CUST_EINTF_TRIGGER_HIGH);
				mutex_lock(&obj->update_lock);
				hwmsen_write_byte(client,REG_CFG1,
...
static ssize_t pa122_show_ps(struct device_driver *ddri, char *buf)
{
    ...
    if(res = pa122_read_ps(pa122_obj->client, &data))
    {
...
static ssize_t pa122_show_psthd(struct device_driver *ddri, char *buf)
{
    int idx,mask;
    if(!pa122_obj)
    {
    ...
    mask = atomic_read(&pa122_obj->ps_thd_val_high);

    return snprintf(buf, PAGE_SIZE, "%d\n", mask);
    //return snprintf(buf, PAGE_SIZE, "%d\n", pa122_obj->ps_thd_val_high);
}

/*----------------------------------------------------------------------------*/
static ssize_t pa122_store_psthd(struct device_driver *ddri, char *buf, size_t count)
{
    int  thres;
    if(!pa122_obj)
    {
    ...
    if(1 == sscanf(buf, "%d",&thres))
    {
        if(thres > 900)
            thres = 900;
        //atomic_set(&pa122_obj->ps_thd_val, thres);
        atomic_set(&pa122_obj->ps_thd_val_high, thres);
        atomic_set(&pa122_obj->ps_thd_val_low, (thres - 80));
    }
    else
    {
        APS_ERR("invalid content: '%s', length = %zd\n", buf, count);
...
static DRIVER_ATTR(psthd,   S_IWUSR | S_IRUGO, pa122_show_psthd, pa122_store_psthd);
/*----------------------------------------------------------------------------*/
static struct device_attribute *pa122_attr_list[] = {
        &driver_attr_ps,
            &driver_attr_psthd,
};
...
static int pa122_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = pa122_get_cust_alsps_hw();//get custom file data struct
...
{
	//APS_FUN(); 
	struct alsps_hw *hw = pa122_get_cust_alsps_hw();
