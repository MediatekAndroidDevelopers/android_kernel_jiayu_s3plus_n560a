...
extern struct alsps_hw *ap3xx6_get_cust_alsps_hw(void);
...
//	if ((err < 0) || err & 0x20) {  //modified for high sensitivity ir data
	if(err < 0) {
...
static ssize_t ap3xx6_show_psthd(struct device_driver *ddri, char *buf)
{
    int idx;
    if(!ap3xx6_obj)
    {
    ...
    return snprintf(buf, PAGE_SIZE, "%d\n", atomic_read(&ap3xx6_obj->ps_thd_val_h));
}

static ssize_t ap3xx6_store_psthd(struct device_driver *ddri, char *buf, size_t count)
{
    int  thres;
    if(!ap3xx6_obj)
    {
    ...
    if(1 == sscanf(buf, "%d",&thres))
    {
        if(thres > 900)
            thres = 900;
        //atomic_set(&ap3xx6_obj->ps_thd_val, thres);
        atomic_set(&ap3xx6_obj->ps_thd_val_h, thres);
        atomic_set(&ap3xx6_obj->ps_thd_val_l, (thres - 80));
    }
    else
    {
...
static int ap3xx6_show_ps(struct device_driver *ddri, char *buf)
{
    ssize_t res;
    u16 data = 0;
    ...
    if(res = ap3xx6_read_ps(ap3xx6_obj->client, &data))
    {
...
/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(ps,      S_IWUSR | S_IRUGO, ap3xx6_show_ps,    NULL);
static DRIVER_ATTR(psthd,   S_IWUSR | S_IRUGO, ap3xx6_show_psthd, ap3xx6_store_psthd);
/*----------------------------------------------------------------------------*/
...
static int ap3xx6_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	mutex_init(&obj->lock);
	obj->hw = ap3xx6_get_cust_alsps_hw();
...
static int ap3xx6_local_init(void)
{
	struct alsps_hw *hw = ap3xx6_get_cust_alsps_hw();
