...
#define AF_DRVNAME "FM50IMX164AF"
#define I2C_SLAVE_ADDRESS        0x1c
#define I2C_REGISTER_ID            0x1c
#define PLATFORM_DRIVER_NAME "lens_actuator_FM50IMX164AF"
#define AF_DRIVER_CLASS_NAME "actuatordrv_FM50IMX164AF"

#define FM50IMX164AF_DEBUG
#ifdef FM50IMX164AF_DEBUG
#define FM50IMX164AFDB printk
#else
#define FM50IMX164AFDB(x,...)
#endif
...
static int g_sr = 3;
static int g_FM50IMX164AF_addr_switch = 0;

static int s4AF_ReadReg(unsigned short *a_pu2Result)
{
    int  i4RetValue = 0;
    char pBuff[2];
    if (g_FM50IMX164AF_addr_switch==0)
    {
    	pBuff[0] = 0x03;
		  ...
		  i4RetValue = i2c_master_send(g_pstAF_I2Cclient, pBuff, 1);
	   	...
	   	if (i4RetValue < 0) 
	  	{
		    FM50IMX164AFDB("[FM50IMX164AF] I2C write failed!! g_FM50IMX164AF_addr_switch=%x\n",g_FM50IMX164AF_addr_switch);
				return -1;
			}	
...
static int s4AF_WriteReg(u16 a_u2Data)
{
  int  i4RetValue = 0;
	...
	if (g_FM50IMX164AF_addr_switch==0)
	{
			...
			if (i4RetValue < 0) 
			{
				FM50IMX164AFDB("[FM50IMX164AF] I2C read failed!! g_FM50IMX164AF_addr_switch=%x\n",g_FM50IMX164AF_addr_switch);
				return -1;
			}
	...
	{
    	g_pstAF_I2Cclient->addr = 0x18>>1;
		  puSendCmd1[0] = (char)(a_u2Data >> 8) | 0xf4;//del for 4s
			puSendCmd1[1] = (char)a_u2Data;//del for 4s
			//FM50IMX164AFDB("[FM50IMX164AF] g_sr %d, write %d \n", g_sr, a_u2Data);
			g_pstAF_I2Cclient->ext_flag |= I2C_A_FILTER_MSG;
...
static int s4FM50IMX164AF_enable(int enable)
{
    int  i4RetValue = 0;

    char puSendCmd[2] = {0x02};
	//s4FM50IMX164AF_ReadID();
    //FM50IMX164AFDB("[FM50IMX164AF] g_sr %d, write %d \n", g_sr, a_u2Data);
    ...
    if (i4RetValue < 0) 
    {
        FM50IMX164AFDB("[FM50IMX164AF] I2C send failed!! \n");
        return -1;
    }

    return 0;
}
inline static int getAFInfo(__user stFM50IMX164AF_MotorInfo * pstMotorInfo)
{
    stFM50IMX164AF_MotorInfo stMotorInfo;
    ...
    if(copy_to_user(pstMotorInfo , &stMotorInfo , sizeof(stFM50IMX164AF_MotorInfo)))
    {
...
#ifdef LensdrvCM3
inline static int getAFMETA(__user stFM50IMX164AF_MotorMETAInfo * pstMotorMETAInfo)
{
    stFM50IMX164AF_MotorMETAInfo stMotorMETAInfo;
    ...
    if(copy_to_user(pstMotorMETAInfo , &stMotorMETAInfo , sizeof(stFM50IMX164AF_MotorMETAInfo)))
    {
        LOG_INF("copy to user failed when getting motor information \n");
    }
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case FM50IMX164AFIOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stFM50IMX164AF_MotorInfo *)(a_u4Param));
        break;
        #ifdef LensdrvCM3
        case FM50IMX164AFIOC_G_MOTORMETAINFO :
            i4RetValue = getAFMETA((__user stFM50IMX164AF_MotorMETAInfo *)(a_u4Param));
        break;
        #endif
        case FM50IMX164AFIOC_T_MOVETO :
            i4RetValue = moveAF(a_u4Param);
        break;

        case FM50IMX164AFIOC_T_SETINFPOS :
            i4RetValue = setAFInf(a_u4Param);
        break;

        case FM50IMX164AFIOC_T_SETMACROPOS :
...
static int AF_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    if(g_s4AF_Opened == 0){
		FM50IMX164AFDB("[FM50IMX164AF] FM50IMX164AF_Open - init lens\n");
		iic_ret = s4FM50IMX164AF_enable(1);
		if(iic_ret == -1)
			g_FM50IMX164AF_addr_switch = 1;  
		else
			g_FM50IMX164AF_addr_switch = 0;
    }
...
static int __init FM50IMX164AF_i2C_init(void)
{
...
static void __exit FM50IMX164AF_i2C_exit(void)
{
    platform_driver_unregister(&g_stAF_Driver);
}

module_init(FM50IMX164AF_i2C_init);
module_exit(FM50IMX164AF_i2C_exit);

MODULE_DESCRIPTION("FM50IMX164AF lens module driver");
