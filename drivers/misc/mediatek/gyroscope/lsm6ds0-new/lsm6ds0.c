...
/*----------------------------------------------------------------------------*/
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, ssize_t count)
{
	struct LSM6DS0_i2c_data *obj = obj_i2c_data;
	int trace;
	if (obj == NULL)
...
#ifdef GPIO_GYRO_EINT_PIN
  mt_set_gpio_mode(GPIO_GYRO_EINT_PIN, GPIO_MODE_00);
  mt_set_gpio_dir(GPIO_GYRO_EINT_PIN, GPIO_DIR_OUT);
  mt_set_gpio_out(GPIO_GYRO_EINT_PIN, GPIO_OUT_ZERO);
#endif
...
static void LSM6DS0_late_resume(struct early_suspend *h)
{
  ...
  mt_set_gpio_mode(GPIO_GYRO_EINT_PIN, GPIO_MODE_00);
  mt_set_gpio_dir(GPIO_GYRO_EINT_PIN, GPIO_DIR_OUT);
  mt_set_gpio_out(GPIO_GYRO_EINT_PIN, GPIO_OUT_ONE);
#endif
...
static int LSM6DS0_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = lsm6ds0_get_cust_gyro_hw();
