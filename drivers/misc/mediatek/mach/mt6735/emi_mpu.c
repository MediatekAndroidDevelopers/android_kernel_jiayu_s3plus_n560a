...
static const struct mst_tbl_entry mst_tbl[] =
{
    ...
    { .master = MST_ID_APMCU_18, .port = 0x0, .id_mask = 0b1111110111111, .id_val = 0b0000000010010, .name = "CQ_DMA" },
    ...
    { .master = MST_ID_PERI_15, .port = 0x1, .id_mask = 0b1111111101111, .id_val = 0b0000000000010, .name = "CQ_DMA" },
    { .master = MST_ID_PERI_16, .port = 0x1, .id_mask = 0b1111111101111, .id_val = 0b0000000000111, .name = "DebugTop" },
...
char *smi_larb0_port[14] = {"disp_ovl_0", "disp_rdma_0", "disp_wdma_0", "disp_ovl_1", "disp_rdma_1", "disp_wdma_1", "ufod_rdma0", "ufod_rdma1", "ufod_rdma2", "ufod_rdma3", "ufod_rdma4", "ufod_rdma5", "ufod_rdma6", "ufod_rdma7", "mdp_rdma", "mdp_wdma", "mdp_wrot"};
...
const unsigned int DM_STATUS[] = {EMI_MPUP, EMI_MPUQ, EMI_MPUR, EMI_MPUY, EMI_MPUP2, EMI_MPUQ2, EMI_MPUR2, EMI_MPUY2};
static void __clear_emi_mpu_vio(void)
{
    u32 dbg_s, dbg_t, i;
    
    /* clear violation status */
    for(i = 0; i < sizeof(DM_STATUS) / sizeof(unsigned int); i++)
        mt_reg_sync_writel(0x00FF03FF, DM_STATUS[i]);
...
static irqreturn_t mpu_violation_irq(int irq, void *dev_id)
{
    ...
    dbg_pqry = readl(IOMEM(DM_STATUS[domain_ID]));
  ...
  #endif
#ifdef CONFIG_MTK_AEE_FEATURE
    /*FIXME: skip ca17-read violation to trigger root-cause KE*/
...
static int __init emi_mpu_mod_init(void)
{
    ...
    ret = request_irq(APARM_DOMAIN_IRQ_BIT_ID, (irq_handler_t)mpu_violation_irq, IRQF_TRIGGER_LOW | IRQF_SHARED, "mt_emi_mpu", &emi_mpu_ctrl); 
    if (ret != 0) {
