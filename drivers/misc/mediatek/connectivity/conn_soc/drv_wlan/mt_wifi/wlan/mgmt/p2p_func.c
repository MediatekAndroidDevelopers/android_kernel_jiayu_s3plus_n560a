...
{
    if(prAdapter && prAdapter->prGlueInfo && (prAdapter->prGlueInfo->ulFlag & GLUE_FLAG_HALT))
		return;
    do {
        ASSERT_BREAK((prAdapter != NULL) &&
                                            (prP2pBssInfo != NULL) &&
            ...
            case ELEM_ID_HT_CAP:    /* 45 */    /* V */
                {
                    ...
                    DBGLOG(P2P, TRACE, ("HT Extended Cap Info:%x, TX Beamforming Cap Info:%x, Ant Selection Cap Info%x \n",
...
/* More than 2 attributes. */
                        ...
                        if (u2CopyLen) {
                            kalMemCopy((PUINT_8)((ULONG)pucAttriListStart + (UINT_32)u2AttriListLen), 
    ...
    )
{
    ...
    do {
        ...
        if (fgIsP2PIE) {
			...
			u2EstimatedExtraIELen += p2pFuncCalculateP2P_IE_NoA(prAdapter, NULL, NULL);
        }
        ...
        if (fgIsWSCIE) {
            kalP2PGenWSC_IE(prAdapter->prGlueInfo,
                                2,
                                (PUINT_8)((ULONG)prRetMsduInfo->prPacket + (UINT_32)prRetMsduInfo->u2FrameLength));
        ...
        for (i = 0; i < u4AttriTableSize; i++) {
            ...
            if (arAppendAttriTable[i].pfnAppendAttri) {
                ...
                if (u4OverallAttriLen > P2P_MAXIMUM_ATTRIBUTE_LEN) {
                    ...
                    pucBuffer = (PUINT_8)((ULONG)prIeP2P + (VENDOR_OUI_TYPE_LEN + P2P_MAXIMUM_ATTRIBUTE_LEN));
    ...
    )
{
    ...
    pucBuffer = (PUINT_8)((ULONG)pucBuf +
                            (UINT_32)(*pu2Offset));
